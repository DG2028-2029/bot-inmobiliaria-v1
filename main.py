...
def seleccion_idioma(cliente_id):
    ...
    return render_template("bienvenida.html", cliente=vendedor, textos=textos, destino="formulario")
...
@app.route("/historial/idioma/<cliente_id>")
def seleccion_idioma_vendedor(cliente_id):
    """Pantalla de idioma intermedia para el vendedor."""
    id_clean = cliente_id.lower()
    vendedor = CLIENTES.get(id_clean)
    ...
    lang = session.get('idioma', 'es')
    textos = DICCIONARIO.get(lang, DICCIONARIO['es'])
    return render_template("bienvenida.html", cliente=vendedor, textos=textos, destino="historial")
...
def login(cliente_id):
    ...
    if request.method == "POST":
        if request.form.get("usuario") == vendedor["usuario"] and request.form.get("password") == vendedor["password"]:
            ...
            # Redirigir a la selección de idioma antes del historial
            return redirect(url_for('seleccion_idioma_vendedor', cliente_id=id_clean))
