...
def seleccion_idioma(cliente_id):
    """Puerta de entrada para LEADS (Clientes)."""
...
# --- NUEVA RUTA PARA SELECCIÓN DE IDIOMA ANTES DEL LOGIN ---

@app.route("/access/<cliente_id>")
def seleccion_idioma_login(cliente_id):
    """Pantalla de selección de idioma exclusiva para el Vendedor antes de loguearse."""
    id_clean = cliente_id.lower()
    vendedor = CLIENTES.get(id_clean)
    ...
    # Aquí usarías una plantilla similar a bienvenida.html pero que redirija a 'login'
    return render_template("bienvenida_login.html", cliente=vendedor, textos=textos)
